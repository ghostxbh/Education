...
public class VeDate {
 ...
 public static String getLastDateOfString(String yearMonth){
	 Date date = null;
	 try {
		date = YEAR_MONTH.parse(yearMonth);
 ...
 */
 ...
 /**
  * 使用当前月份,得到下一个月的月份:月份的格式是:yyyyMM
  * @Author Axin
  * @DateTime 2018年9月30日 下午4:51:51
  * @param currentDate
  * @return
  */
 public static String getNextMonthOfShort(String currentDate) {
...
public static String getLastDateByPeriod(String yearMonth){
		Date date = null;
		try {
			date = YEAR_MONTH_OF_SHORT.parse(yearMonth);
		} catch (ParseException e) {
			e.printStackTrace();
		}
		Calendar cal = Calendar.getInstance();
 ...
 * 获取指定月份前的本年月份
 * @Author Axin
 * @DateTime 2018年10月11日 下午6:05:52
 * @param month  格式yyyyMM
 * @return
 */
public static String[] getCurYearBeforeMonths(String month){
	if(StringUtils.isEmpty(month)){
		throw new NullPointerException("月份不能为空");
	}
	Date lastDate = null;
	try {
		lastDate = YEAR_MONTH_OF_SHORT.parse(month);
	} catch (ParseException e) {
		e.printStackTrace();
		throw new ParamIllegalException(" 日期格式应为yyyyMM");
	}
	
	String curMonth = month.substring(0,4)+"01";
	Calendar calendar = Calendar.getInstance();
	Date curDate = null;
	try {
		curDate = YEAR_MONTH_OF_SHORT.parse(curMonth);
	} catch (ParseException e) {
		e.printStackTrace();
	}
	...
	while(calendar.getTimeInMillis()<=lastDate.getTime()){
		String str = YEAR_MONTH_OF_SHORT.format(calendar.getTime());
		months.add(str);
		calendar.add(Calendar.MONTH, 1);
	}
	
	String[] monthArr = months.toArray(new String[months.size()]);   
	return monthArr;
 ...
 public static void main(String[] args) throws Exception {
	...
	String str = getFirstDateByPeriod("201801");
	String str2 = getLastDateByPeriod("201811");
	String str3 = getLastDateOfString("2018-11");
