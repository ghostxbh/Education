    ...
    <script>
                ...
                success: function (data) {
                    ...
                    if (courseList.length > 0) {
                        for (var i = 0; i < courseList.length; i++) {
                            ...
                            var course_price = courseList[i].price;
                            var course_lecturer = courseList[i].lecturer;
                            var course_id = courseList[i].id;
                            var course_name = courseList[i].courseName;
                            var course_info = courseList[i].courseInfo;
                            var course_create_date = courseList[i].insertTime;
                            //显示产品数据
                            tbody.append("<tr><td><input type='checkbox' class='cbx_select' value='"+course_id+"' id='cbx_course_select_" + course_id + "'><label for='cbx_course_select_" + course_id + "'></label></td><td title='"+course_name+"'>" + course_name + "</td><td title='"+course_info+"'>" + course_info + "</td><td title='"+course_price+"'>" + course_price + "</td><td title='"+course_lecturer+"'>" + course_lecturer + "</td><td title='"+course_create_date+"'>" + course_create_date + "</td><td><span class='" + isEnabledClass + "' title='"+isEnabledTitle+"'>"+ isEnabled + "</span></td><td><span class='td_special' title='查看产品详情'><a href='javascript:void(0);' onclick='getChildPage(this)'>详情</a></span></td><td hidden><span class='course_id'>" + course_id + "</span></td></tr>");
                        }
                        //绑定事件
        ...
        function getChildPage(obj) {
            ...
            if(obj === null){
                ...
                title = "产品详情";
                url = "course/"+$(obj).parents("tr").find(".course_id").text();
            }
...
<div class="frm_div text_info">
    ...
    <div class="frm_group">
        ...
        <label class="frm_label"  id="lbl_course_sale_price" for="input_course_sale_price">金额</label>
        <input class="frm_input frm_num"  id="input_course_sale_price" type="text" placeholder="最低价" maxlength="10">
        <span id="text_cut">—</span>
        <input class="frm_input frm_num"  id="input_course_price" type="text" placeholder="最高价" maxlength="10">
        <span class="frm_error_msg" id="text_course_msg"></span>
    </div>
...
<div class="table_normal_div">
    <table class="table_normal" id="table_course_list">
        ...
        <c:forEach items="${requestScope.courseList}" var="course">
            <tr>
                <td><input type="checkbox" class="cbx_select" id="cbx_course_select_${course.id}" value="${course.id}"><label for="cbx_course_select_${course.id}"></label></td>
